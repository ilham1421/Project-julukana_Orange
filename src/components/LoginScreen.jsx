      ...
      >
        <Card className="border-0 shadow-xl rounded-lg">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardTitle className="text-3xl font-bold text-amber-700">Login CAT JULUKANA</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="fullName" className="text-slate-700">Nama Lengkap (Tanpa Gelar)</Label>
                <div className="relative">
                  <User className="w-5 h-5 absolute left-3 top-2.5 text-slate-400" />
                  <Input id="fullName" type="text" placeholder="Contoh: Andi Julukana"
                    className={`pl-10 border-slate-300 focus:border-amber-500 focus:ring-amber-500 ${errors.fullName ? 'border-red-500' : ''}`}
                    value={fullName} onChange={handleFullNameChange} />
                </div>
                {errors.fullName && <p className="text-sm text-red-500 flex items-center gap-1"><AlertCircle size={16}/>{errors.fullName}</p>}
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="nip" className="text-slate-700">NIP (Nomor Induk Pegawai)</Label>
                <div className="relative">
                  <KeyRound className="w-5 h-5 absolute left-3 top-2.5 text-slate-400" />
                  <Input id="nip" type="text" placeholder="Masukkan 18 digit NIP"
                    className={`pl-10 border-slate-300 focus:border-amber-500 focus:ring-amber-500 ${errors.nip ? 'border-red-500' : ''}`}
                    value={nip} onChange={handleNipChange} maxLength="18" />
                </div>
                {errors.nip && <p className="text-sm text-red-500 flex items-center gap-1"><AlertCircle size={16}/>{errors.nip}</p>}
              </div>
